...
def create_mcp_interface():
    """Create a Gradio interface specifically optimized for MCP using Blocks."""
    ...
    with gr.Blocks(
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        ...
                        paper_start_year = gr.Number(
                            label="Start Year (optional)", 
                            minimum=1900, 
                            maximum=2030,
                            precision=0
                        )
                        paper_end_year = gr.Number(
                            label="End Year (optional)", 
                            minimum=1900, 
                            maximum=2030,
                            precision=0
                        )
                    paper_search_btn = gr.Button("Search Papers", variant="primary")
                
                with gr.Column(scale=3):
                    paper_results = gr.JSON(label="Paper Results")
            
            paper_search_btn.click(
                search_openalex_papers,
                inputs=[paper_query, paper_max_results, paper_start_year, paper_end_year],
                outputs=paper_results
            )
        
        with gr.Tab("Get by DOI"):
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    doi_search_btn = gr.Button("Get Publication", variant="primary")
                
                with gr.Column(scale=3):
                    doi_results = gr.JSON(label="Publication Details")
            
            doi_search_btn.click(
        ...
        with gr.Tab("Search Authors"):
            with gr.Row():
                with gr.Column(scale=2):
                    author_query = gr.Textbox(
                        label="Author Name",
                        placeholder="Enter author name...",
                        value="Geoffrey Hinton"
                    )
                    author_max_results = gr.Number(
                        label="Max Results", 
                        value=5, 
                        minimum=1, 
                        maximum=20,
                    ...
                    author_search_btn = gr.Button("Search Authors", variant="primary")
                
                with gr.Column(scale=3):
                    author_results = gr.JSON(label="Author Results")
            
            author_search_btn.click(
                search_openalex_authors,
                inputs=[author_query, author_max_results],
                outputs=author_results
            )
        
        with gr.Tab("Search Concepts"):
            with gr.Row():
                with gr.Column(scale=2):
                    concept_query = gr.Textbox(
                        label="Concept Name",
                        placeholder="Enter concept/field name...",
                        value="artificial intelligence"
                    )
                    concept_max_results = gr.Number(
                        label="Max Results", 
                        value=5, 
                        minimum=1, 
                        maximum=20,
                    ...
                    concept_search_btn = gr.Button("Search Concepts", variant="primary")
                
                with gr.Column(scale=3):
                    concept_results = gr.JSON(label="Concept Results")
            
            concept_search_btn.click(
                search_openalex_concepts,
                inputs=[concept_query, concept_max_results],
        ...
        with gr.Tab("MCP Configuration"):
            ...
            ### Search Authors
            ```python
            search_openalex_authors("Geoffrey Hinton", 3)
            ```
            
            ### Search Concepts
            ```python
            search_openalex_concepts("machine learning", 10)
            ```
            """)
